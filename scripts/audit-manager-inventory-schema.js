...
async function audit() {
  ...
  const [inventory, orders, receipts, authorizations, tickets, inspections, loans, users, suppliers, materials, dispatches, serializedAssets] = await Promise.all([
    ...
    db.collection('service_tickets').find({}).toArray(),
    db.collection('inspectiontickets').find({}).toArray(),
  ...
  const report = {
    ...
    inspectionTickets: {
      ...
      danglingCustomer: inspections.filter((ticket) =>
        ticket.customerId && !userIds.has(String(ticket.customerId))).length,
    },
    assetLoans: {
      total: loans.length,
