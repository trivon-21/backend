...
const {
  ...
  sendSchedulingEmail,
  sendReminderEmail,
} = require("../notification/email.service");
const { createLog } = require("../../finance/auditLog.controller");
...
const getOrderModel = () => {
  try { return mongoose.model("InstallationOrder"); }
  catch {
    const s = new mongoose.Schema({
    ...
    return mongoose.model("InstallationOrder", s, "installation_orders");
  }
};
...
exports.getOrCreateTicket = async (req, res) => {
  try {
    ...
    console.log("=== DEBUG getOrCreateTicket ===");
    console.log("Looking up orderId:", orderId);

    const InspectionTicket = getTicketModel();
    const Order = getOrderModel();
    console.log("Order model name:", Order.modelName);
    console.log("Order model collection:", Order.collection.collectionName);
...
exports.getPendingVerification = async (req, res) => {
  try {
    ...
    const enriched = await Promise.all(tickets.map(async (t) => {
      ...
      obj.ticketId = t.ticketRef || `I-Tic-${t._id.toString().slice(-5).toUpperCase()}`;
      if (t.customerId) {
...
// ── GET verified payments — match by exclusion, not exact whitelist ──────────
