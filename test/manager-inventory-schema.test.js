...
test('adapted models use the shared collection names', () => {
  ...
  assert.equal(InspectionTicket.collection.collectionName, 'inspectiontickets');
...
test('incoming schemas remain isolated from current-system model registrations', () => {
  ...
  assert.notEqual(InspectionTicket.modelName, LegacyInspectionTicket.modelName);
  ...
  assert.equal(InspectionTicket.modelName, 'ManagerInspectionTicket');
